            ...
            <Marquee pauseOnHover={true} className='py-1' >
                {
                    posts.map((animal: any, index: number) => {
                        return (
                            <Link href={`/entity/${animal.id}`} key={`${animal.id}-${index}`} className='grid grid-cols-[auto_1fr] place-items-center gap-2 p-1 bg-white shadow-sm mx-1'>
                                ...
                                <div className='text-xs'>
                                    ...
                                    <div className='text-zinc-700 italic line-clamp-2 max-w-32'>{formalizeText(animal.description)}</div>
