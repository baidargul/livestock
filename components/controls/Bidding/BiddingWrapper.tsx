...
const BiddingWrapper = (props: Props) => {
    ...
    const secondLayer = dialog.secondLayer
    ...
    const handleOpen = (val: boolean) => {
        if (secondLayer === "bidding" && val === false) {
            dialog.setSecondLayer(val ? "bidding" : "")
        }
        if (val === false) {
    ...
    return (
        animal && <>
            <div className={`fixed ${props.staticStyle ? 'bottom-0 h-[95%]' : 'bottom-0 h-[80%]'}  select-none flex flex-col justify-between gap-0 ${secondLayer === "bidding" ? "translate-y-0 pointer-events-auto opacity-100 z-50" : " z-30 translate-y-full pointer-events-none opacity-0"} transition-all duration-300 drop-shadow-2xl border border-emerald-900/30 w-[96%] mx-2 left-0 rounded-t-xl bg-white p-4`}>
                ...
                <div className={`flex flex-col gap-4 ${activeBidRoom ? "bg-gradient-to-b from-amber-50 to-transparent" : "bg-white"} `}>
                    ...
                    {!activeBidRoom?.userOfferAccepted && <div className='overflow-y-auto h-full max-h-[400px]' style={{ pointerEvents: isLocked && activeBidRoom ? "none" : "auto" }}>
                        {!activeBidRoom && <Rooms rooms={[...rooms?.myRooms, ...rooms?.otherRooms]} socket={socket} targetRoomKey={props.targetRoomKey} isOpen={secondLayer === "bidding"} setExpectedKey={setExpectedKey} expectedKey={expectedKey} currentUser={user} animal={animal ?? null} isStaticStyle={props.staticStyle ?? false} />}
                        {
            ...
            <div onClick={handleCreateBidRoom} className={`w-full ${secondLayer === "bidding" && "pointer-events-none opacity-50 scale-75"} w-full transition-all duration-100 ease-in-out`}>
            ...
            <div onClick={() => handleLeaveRoom(true)} className={`fixed ${secondLayer === "bidding" ? "pointer-events-auto opacity-100 backdrop-blur-[1px]" : "pointer-events-none opacity-0"} top-0 left-0 inset-0 w-full h-full bg-black/50 z-10`}></div>
        </>
    )
}
