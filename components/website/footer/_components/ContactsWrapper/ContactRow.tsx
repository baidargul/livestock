    ...
    return (
        <div className='w-full p-1 pb-2 border-b border-zinc-300'>
            <div className='flex justify-between items-center'>
                <div className='text-lg font-semibold'>{props.contact.user.name}</div>
                <div className='italic text-sm'>{props.contact.remarks}</div>
            </div>
            <div className='text-xs'>{new Date(props.contact.createdAt).toDateString()} - {new Date(props.contact.createdAt).toLocaleTimeString()}</div>
